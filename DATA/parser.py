...
def load_parsed_file(file_sufix, occur, s_letter, p_letter, current_id):
    paragraph_path = f'DATA/raw_files/{file_sufix}.txt'
    summary_path = f'DATA/raw_files/{file_sufix}_summary.txt'
    ...
    with open(
            f'DATA/parsed_files/{file_sufix}_train.json',
            'w') as file:
        file.write(json.dumps(parsed_file_train, ensure_ascii=False))

    with open(
            f'DATA/parsed_files/{file_sufix}_test.json',
            'w') as file:
        file.write(json.dumps(parsed_file_test, ensure_ascii=False))

    with open(
            f'DATA/parsed_files/{file_sufix}_validate.json',
            'w') as file:
...
def concatenate_files_split(split):
    ...
    json_files = glob.glob(
        f'DATA/parsed_files/*_{split}.json')
    print(json_files)
    ...
    with open(f"DATA/parsed_data_splits/rabbi_kook_{split}.json", "w") as f:
